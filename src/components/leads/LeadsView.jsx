...
import { fetchExecutionStatus, analyzeSummaryWithDeepSeek } from '../../services/api';
...
export const LeadsView = ({ 
  ...
  const combinedContacts = useMemo(() => {
    ...
    if (contacts && contacts.length > 0) {
      contacts.forEach(c => {
        ...
        // If the database version already has summary/category, but session version does not, keep the database version!
        if (existing && existing.leadCategory && existing.summary && (!c.leadCategory || !c.summary)) {
          // Keep existing db contact
        } else {
          map.set(c.id, c);
        }
      });
  ...
  const handleCategoryClick = useCallback(async (contact) => {
    const isPending = !contact.summary || !contact.leadCategory || contact.leadCategory === 'PENDING' || contact.leadCategory === 'Pending';

    setModalData({
      id: contact.id,
      name: contact.name,
      phone: contact.phone,
      category: contact.leadCategory || 'PENDING',
      summary: contact.summary || 'Fetching real-time status and generating AI summary...',
      recordingUrl: contact.recordingUrl || null,
      recovering: isPending && !!contact.executionId
    });

    let currentRecordingUrl = contact.recordingUrl || null;
    let currentSummary = contact.summary || '';
    let currentCategory = contact.leadCategory || '';

    // If it's pending / missing summary, and we have an executionId, let's recover it!
    if (isPending && contact.executionId && apiKey) {
      try {
        const data = await fetchExecutionStatus(apiKey, contact.executionId);
        if (data) {
          currentRecordingUrl = data.telephony_data?.recording_url || currentRecordingUrl;
          if (data.summary) {
            currentSummary = data.summary;
            // Run AI analysis
            const analyzedCat = await analyzeSummaryWithDeepSeek(DEEPSEEK_API_KEY, data.summary);
            currentCategory = analyzedCat || 'Uncategorized';
          } else {
            currentSummary = 'Call is completed, but summary has not been generated by Bolna yet. Please click again in a few seconds.';
          ...
          const updatedContact = {
            ...
            status: contact.status === 'calling' || contact.status === 'queued' ? 'called' : contact.status
          };
          ...
          // Update local dbContacts state so the list updates live instantly
          setDbContacts(prev => prev.map(c => c.id === contact.id ? updatedContact : c));

          // Update the open modal details
          setModalData(prev => prev && prev.id === contact.id ? {
            ...prev,
          ...
          setModalData(prev => prev && prev.id === contact.id ? { ...prev, recovering: false } : prev);
        }
      } catch (err) {
        console.error('Failed to recover lead status:', err);
        setModalData(prev => prev && prev.id === contact.id ? { 
          ...prev, 
          summary: contact.summary || 'Failed to fetch status from Bolna. Please try again.',
          recovering: false 
        } : prev);
      }
    } else {
      // Standard flow: if no recording URL stored but we have executionId and apiKey, try fetching recording
      if (!contact.recordingUrl && contact.executionId && apiKey) {
        setLoadingRecording(true);
        try {
          const data = await fetchExecutionStatus(apiKey, contact.executionId);
          if (data && data.telephony_data && data.telephony_data.recording_url) {
            setModalData(prev => prev && prev.id === contact.id ? { ...prev, recordingUrl: data.telephony_data.recording_url } : prev);
          }
        } catch (e) {
          ...
          <div className="category-detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="category-modal-section">
              ...
              <div className="category-summary-block">
                {modalData.recovering ? (
                  <div className="summary-loading" style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'rgba(255,255,255,0.7)' }}>
                    <div className="audio-loading-spinner" />
                    <span>Analyzing call data and generating summary...</span>
                  </div>
                ) : (
                  modalData.summary
                )}
